(() => {
  ...
  document.querySelectorAll("[data-v17-feedback-form]").forEach(form => {
    ...
    const satisfactionInputs=[...form.querySelectorAll(
      "input[name='memnuniyet'], input[name='sorun_cozuldu'], input[name='hizmet_hizi'], input[name='bilgilendirme']"
    )];
    ...
    form.addEventListener("submit",event => {
      ...
      if(result==="bilgilendirildi"){
        ...
        const solved=form.querySelector("input[name='sorun_cozuldu']:checked");
        const speed=form.querySelector("input[name='hizmet_hizi']:checked");
        const info=form.querySelector("input[name='bilgilendirme']:checked");
        const minutes=Number(duration?.value || 0);

        if(!rating || !solved || !speed || !info || !Number.isFinite(minutes) || minutes<1){
          event.preventDefault();
          alert("Görüşme tamamlandıysa vatandaş değerlendirmelerini ve işlem süresini doldurun.");
          return;
        }
