(() => {
  ...
  function csrfTokenFor(el) {
    return el?.closest("[data-callback-detail]")?.querySelector(
      "input[name='csrfmiddlewaretoken']"
    )?.value || "";
  }

  function enableCallResults(detail, startedAt) {
    const form = detail?.querySelector("[data-call-result-form]");
    if (!form) return;
    form.dataset.callActive = "1";
    form.querySelectorAll("[data-call-result-button]").forEach(btn => {
      btn.disabled = false;
    });
    const status = form.querySelector("[data-call-status]");
    if (status) {
      status.textContent = `Arama oturumu ${startedAt} itibarıyla kayıt altında. Görüşme sonucunu seçin.`;
    }
  }

  document.querySelectorAll("[data-call-start]").forEach(button => {
    button.addEventListener("click", async () => {
      if (button.disabled) return;
      const detail = button.closest("[data-callback-detail]");
      const url = button.dataset.callStartUrl;
      const phone = button.dataset.callPhone || "";
      const csrf = csrfTokenFor(button);

      button.disabled = true;
      const oldText = button.textContent;
      button.textContent = "Arama kaydı oluşturuluyor...";
      ...
      try {
        ...
        if (!response.ok || !data.ok) {
          alert(data.message || "Arama oturumu başlatılamadı.");
        ...
        button.textContent = `✓ Arama Kaydı Açık • ${data.started_at}`;
        enableCallResults(detail, data.started_at);

        const note = detail?.querySelector("[data-call-proof-note]");
        if (note) {
          note.textContent =
            "Arama başlangıcı sistem loguna işlendi. Sonuç kaydında bitiş saati ve uygulama oturum süresi de yazılacak.";
        }

        // Telefon/softphone tanımlıysa arama ekranını açar.
        // Açılmasa bile uygulama içi başlangıç kanıtı kaydedilmiş olur.
        if (phone) {
          setTimeout(() => {
        ...
        button.disabled = false;
        button.textContent = oldText;
        alert("Arama oturumu kaydedilirken bağlantı hatası oluştu.");
      }
    });
  });
